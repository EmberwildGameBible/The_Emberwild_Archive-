...
See your branch name (for example, `work`) and confirm nothing is unstaged:
```
git status -sb
```
(Optional) Pull remote updates so you don’t push over old history:
```
git fetch origin
```
Push this branch to GitHub (replace `<branch-name>` with the name from step 1):
```
